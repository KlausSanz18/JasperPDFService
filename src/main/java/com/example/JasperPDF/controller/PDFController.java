...
import com.example.Springboot.viewModel.reports.ProgressReport;
...
public class PDFController {
	...
	List<ProgressReport> progressReport = new LinkedList<ProgressReport>();

	// Iterator<ProjectProgress> ProjectProgress_iterator;
	...
	public String setGeneralAdvance(@RequestBody ProgressReport source) {
		// System.out.println("tengo que ver: "+ source);
		// ProgressReport result = new ProgressReport();

		source.getEquipment().iterator();
		projectProgress.clear();
		progressReport.add(source);
		// System.out.println("ahora si :"+ );
		ProjectProgress pdf = new ProjectProgress();

		List<ProjectProgress> equipos = source.getEquipment();
		List<ProjectProgress> materiales = source.getMaterials();
		List<ProjectProgress> servicios = source.getServices();



		for (ProjectProgress pp : equipos) {
			projectProgress.add(pp);
		}
		for (ProjectProgress pp : materiales) {
			projectProgress.add(pp);
		}
		for (ProjectProgress pp : servicios) {
			projectProgress.add(pp);
		}
